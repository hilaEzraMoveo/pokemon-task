   ...
   getPokemons(): Observable<Pokemon[]>{
         ...
         .pipe(
            mergeMap( response => {
             ...
             const observables: Observable<Pokemon>[] = results.map((pokemonInfo: any, index: number) => {
    ...
    private getPokemonDetails(id: number, url: string): Observable<Pokemon> {
      ...
      getPokemonById(pokemonId: number): Observable<Pokemon> {
        ...
        return this.http.get<any>(`${this.apiUrl}${pokemonId}`).pipe(
          map(pokemonInfo => this.mapPokemon(pokemonInfo))
        );
      }
